	...
	function Flash(root, opts, conf) {
		...
		if ((conf.RenderingOrder.indexOf('flash') == 0 || (conf.RenderingOrder.indexOf('flash')>0 &&!supportsHTML4)) && f.isSupported(opts.version)) {
            if(conf.Toolbar){
                ...
                var calcHeight = parseInt(jQuery(root).css('height'),10) - $(".flexpaper_toolbar").outerHeight(true);
                jQuery(root).height(calcHeight);
            }
			...
			if(true){
				jQuery(document).ready(function() {
					...
					jQuery.getScript(conf.jsDirectory+'FlexPaperViewer.js', function() {
                        ...
                        if(conf.JSONFile != null && conf.JSONFile.length > 0){
						...
						var oRenderingList 	= conf.RenderingOrder.replace(/\s/g, '').split(",");
...
(function checkXMLHttpRequestResponseCompatibility() {
  ...
  // jacekh: fix ie7 http://www.ilinsky.com/articles/XMLHttpRequest/
  // Keep the reference to earlier defined object implementation (if any) 
  var xXMLHttpRequest = window.XMLHttpRequest; // Constructor 
  function XMLHttpRequest() { this.object = xXMLHttpRequest ? new xXMLHttpRequest : new window.ActiveXObject('Microsoft.XMLHTTP'); }
